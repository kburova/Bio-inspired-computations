...
public class BackPropagation {
    ...
    BufferedWriter bw;
    String problemName = "Approximation1";
    private int numberOfInputs = 3;
    private int numberOfLayers = 1;
    private int [] numberOfNeurons = { 15 };
...
//            System.out.print("Provide with: [problem name string][inputs number] [layers number] [epochs number] [learning rate]\n" +
//                    "Separate values by space\n");
//            problemName = scanner.next();
//            numberOfInputs = scanner.nextInt();
//            numberOfLayers = scanner.nextInt();
//            numberOfEpochs = scanner.nextInt();
//            learningRate = scanner.nextDouble();
//            numberOfNeurons = new int[numberOfLayers];
//            System.out.println("type nodes number for each layer: \n");
//            for (int i = 0; i < numberOfLayers; i++){
//                System.out.println("Layer i: ");
//                numberOfNeurons[i] = scanner.nextInt();
//            }
//            scanner.close();
    ...
    private void printRMSE(){
        try{
            ...
            bw.flush();
            System.out.println(RMSE.get(numberOfEpochs-1));
        }catch (IOException e){
            e.printStackTrace();
        }
    }
    public void testNetwork() {
        try{
            ...
            while ( (line = br.readLine()) != null ) {
                ...
                if (Math.abs(expectedOutput - outputSigma) > (RMSE.get(RMSE.size()-1) + 0.01)){
                    bw.write(", no\n");
                    notSolved++;
                }else {
                    bw.write(",yes\n");
